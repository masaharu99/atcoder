...
func main() {
	r, g, b := ScanI(), ScanI(), ScanI()
	c := ScanS()

	if c == "Red" {
		fmt.Println(int(math.Min(float64(g), float64(b))))
	} else if c == "Green" {
		fmt.Println(int(math.Min(float64(r), float64(b))))
	} else {
		fmt.Println(int(math.Min(float64(r), float64(g))))
	}
